...
*"* do not include other source files here!!!
  PUBLIC SECTION.
    ...
    CLASS-DATA o_conv TYPE REF TO if_abap_conv_out.
...
*--------------------------------------------------------------------*
  ...
  METHOD char2hex.
    ...
    IF o_conv IS NOT BOUND.
      o_conv = cl_abap_conv_codepage=>create_out( ).
...
*    r_hex+1 = o_conv->get_buffer( ). " x'65' must be x'0065'
    r_hex+1 = o_conv->convert( EXACT #( i_char ) ).
...
*--------------------------------------------------------------------*
    CONSTANTS lc_abcde TYPE C LENGTH 26 VALUE 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.
...
*      lv_uccpi  = 65 + lv_module.
      ...
      lv_text   = substring( val = lc_abcde off = lv_module len = 1 ).
...
*      lt_dfies = io_struct->get_ddic_field_list( ).
...
* Check if cell is in range
  ...
  METHOD number_to_excel_string.
    ...
    IF ip_currency IS INITIAL.
      ep_value = |{ ip_value sign = left }|.
*      WRITE ip_value TO lv_value_c EXPONENT 0 NO-GROUPING NO-SIGN.
    ELSE.
      ep_value = |{ ip_value CURRENCY = ip_currency sign = left }|.
