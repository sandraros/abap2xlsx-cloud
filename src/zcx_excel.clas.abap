...
*    DATA syst_at_raise TYPE syst .
    TYPES: BEGIN OF ty_sy_message,
             ...
             msgv3 TYPE symsgv,
             msgv4 TYPE symsgv,
           END OF ty_sy_message.
    DATA sy_message TYPE ty_sy_message READ-ONLY.
...
*        !syst_at_raise TYPE syst OPTIONAL .
        !sy_message TYPE ty_sy_message OPTIONAL.
...
*    ENDIF.
  ...
  METHOD if_message~get_text.
    IF me->error IS NOT INITIAL.
      ...
      " ---------------------------------------------------------------------
      result = me->error.
    ELSEIF me->sy_message IS NOT INITIAL.
      "    ELSEIF me->syst_at_raise IS NOT INITIAL.
      " ---------------------------------------------------------------------
      " If message was supplied by syst create messagetext now
      " ---------------------------------------------------------------------
      MESSAGE ID sy_message-msgid TYPE sy_message-msgty NUMBER sy_message-msgno
              WITH  sy_message-msgv1 sy_message-msgv2 sy_message-msgv3 sy_message-msgv4
              INTO  result.
...
*           INTO  result.
  ...
  METHOD raise_symsg.
    RAISE EXCEPTION TYPE zcx_excel
      EXPORTING sy_message = VALUE #( msgid = sy-msgid
                                      msgty = sy-msgty
                                      msgno = sy-msgno
                                      msgv1 = sy-msgv1
                                      msgv2 = sy-msgv2
