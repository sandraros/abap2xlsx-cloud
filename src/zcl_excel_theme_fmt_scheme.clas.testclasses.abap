CLASS ltcl_test DEFINITION FOR TESTING DURATION SHORT RISK LEVEL HARMLESS.
  PRIVATE SECTION.
    ...
    DATA mi_ixml TYPE REF TO zif_excel_ixml.
    DATA mi_document TYPE REF TO zif_excel_ixml_document.
    METHODS setup.
    METHODS render
      RETURNING
...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD setup.
    mi_ixml = zcl_excel_ixml=>create( ).
    mi_document = mi_ixml->create_document( ).
  ENDMETHOD.

  METHOD render.
    DATA li_ostream  TYPE REF TO zif_excel_ixml_ostream.
    DATA li_renderer TYPE REF TO zif_excel_ixml_renderer.
