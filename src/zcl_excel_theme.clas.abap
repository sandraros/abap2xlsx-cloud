CLASS zcl_excel_theme DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS read_theme
      IMPORTING
        VALUE(io_theme_xml) TYPE REF TO zif_excel_ixml_document .
    METHODS write_theme
      RETURNING
...
CLASS zcl_excel_theme IMPLEMENTATION.
  ...
  METHOD read_theme.
    DATA: lo_node_theme TYPE REF TO zif_excel_ixml_element.
    DATA: lo_theme_children TYPE REF TO zif_excel_ixml_node_list.
    DATA: lo_theme_iterator TYPE REF TO zif_excel_ixml_node_iterator.
    DATA: lo_theme_element TYPE REF TO zif_excel_ixml_element.
  ...
  METHOD write_theme.
    DATA: lo_ixml         TYPE REF TO zif_excel_ixml,
          lo_element_root TYPE REF TO zif_excel_ixml_element,
          lo_encoding     TYPE REF TO zif_excel_ixml_encoding.
    ...
    DATA: lo_document TYPE REF TO zif_excel_ixml_document.
    lo_ixml = zcl_excel_ixml=>create( ).

    lo_encoding = lo_ixml->create_encoding( byte_order = zif_excel_ixml_encoding=>co_platform_endian
                                            character_set = 'UTF-8' ).
